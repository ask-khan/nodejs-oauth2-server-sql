...
const getClient = function (clientId, clientSecret, callback) {
    ...
    client.query('SELECT client_id, client_secret, redirect_uri FROM oauth_clients WHERE ' +
        'client_id = $1', [clientId], (err, result) => {
...
const getRefreshToken = function (bearerToken, callback) {
...
var authorizedClientIds = ['application', 'def2'];
...
const saveToken = function (token, clientContent, userId, callback) {
    token.client = { id: clientContent.clientId }
    token.user = { id: userId }
    client.query('INSERT INTO oauth_access_tokens(access_token, client_id, user_id, expires) ' +
        'VALUES ($1, $2, $3, $4)', [token.accessToken, clientContent.clientId, userId, token.accessTokenExpiresAt],
        function (err, result) {
            callback(err, result.rowCount ? token : null);
        });

};

const saveRefreshToken = function (token, clientContent, userId, callback) {

    client.query('INSERT INTO oauth_refresh_tokens(refresh_token, client_id, user_id, ' +
        'expires) VALUES ($1, $2, $3, $4)', [token.refreshToken, clientContent.clientId, userId, token.refreshTokenExpiresAt],
        function (err, result) {
            callback(err);
